        ...
        <form wire:submit.prevent="makeDeposit">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div class="col-span-1" x-data="{ open: false }">
                    ...
                    <div class="relative">
                        <select id="payment"  wire:model.live="selectedPayment" class="block border w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-slate-500 focus:border-slate-500 sm:text-sm rounded-md" {{ empty($payments) ? 'disabled' : '' }}>
                    ...
                    @foreach($paymentAccounts as $account)
                        <label for="account-{{ $account->id }}" class="relative cursor-pointer">
                            ...
                            <div class="border rounded-lg p-4 transition-all duration-200 ease-in-out hover:shadow-md 
                                {{ $selectedPaymentAccount == $account->id ? 'bg-slate-50 border-slate-500 ring-2 ring-slate-500' : 'border-gray-300' }}">
                ...
                <div class="grid grid-cols-1  gap-6 mb-6">
                    <div class="col-span-1">
                        ...
                        <div class="mt-1 relative rounded-md s">
                            <input type="number" id="amount" wire:model="amount" class="focus:ring-slate-500 p-2 border  focus:border-slate-500 block w-full pr-12 sm:text-sm border-gray-300 rounded-md" placeholder="0.00" step="0.01" min="0">
